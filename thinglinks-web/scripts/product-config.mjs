...
const productConfigLockRef = 'refs/thinglinks/product-config-lock/thinglinks-web';
...
const scanExclusions = [
  ...
  // jessibuca-pro 目录对应第三方播放器适配资源。
  'public/resource/jessibuca-pro',
];
...
function buildPackageContent(
  ...
  pkg.license = derivePackageLicense(values.THINGLINKS_LICENSE_MODEL);
...
function assertPackageMetadata(
  ...
  const expectedLicense = derivePackageLicense(values.THINGLINKS_LICENSE_MODEL);
...
function replaceFilesAtomically(projectRoot, changes, lock) {
  ...
  try {
    for (const { relativePath, content } of changes) {
      ...
      prepared.push({ target, temporary, original, mode });
    }
  } catch (error) {
...
export function setVersion(projectRoot = defaultProjectRoot, version) {
  ...
  return withProductConfigLock(projectRoot, (lock) => {
    ...
    console.log(`已将 Web 版本原子更新为 ${version}。`);
    return values;
  });
}
...
export function runCli(args = process.argv.slice(2), projectRoot = defaultProjectRoot) {
  ...
  switch (command) {
    ...
    case 'set-version':
      if (!args[1]) throw productError('用法：pnpm product:set-version <新版本>');

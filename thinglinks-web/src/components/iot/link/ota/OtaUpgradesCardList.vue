      ...
      <a-row v-if="deviceList.length" :gutter="[24, 12]">
        ...
        >
          <div
            :class="{
              'product-item': true,
              normal: record.status == 0,
              error: record.status == 1,
            }"
            @click="selectItem(record, $event)"
          >
            <div class="product-info">
              <div class="status"
                >{{ record.echoMap.status }} | {{ record.echoMap.signMethod }}
              </div>
              <a-tooltip placement="top" :title="record.packageName">
                <div class="title">{{ record.packageName }}</div>
              </a-tooltip>
              <div class="props">
                <div class="flex">
                  <div class="prop">
                    <div class="label">{{ t('iot.link.ota.otaUpgrades.packageType') }}</div>
                    <div class="value">
                      {{ getDictLabel('LINK_OTA_PACKAGES_TYPE', record.packageType + '', '') }}
                    </div>
                  </div>
                  <div class="prop">
                    <div class="label">{{ t('iot.link.ota.otaUpgrades.version') }}</div>
                    <div class="value">{{ record.version }}</div>
                  </div>
                </div>
                <div class="prop">
                  <div class="label">{{ t('iot.link.ota.otaUpgrades.productIdentification') }}</div>
            ...
            <div class="product-img ota-img">
              <img
                v-if="record?.packageType === 0"
                src="/@/assets/images/iot/link/software.png"
                @click="handleView(record, $event)"
              />
              <img
                v-else-if="record?.packageType === 1"
                src="/@/assets/images/iot/link/firmware.png"
                @click="handleView(record, $event)"
              />
            </div>
          </div>
...
<script lang="ts">
  ...
  import EditModal from '/@/views/iot/link/ota/otaUpgrades/Edit.vue';

...
<script lang="ts">
  ...
  import { page, remove } from '/@/api/iot/link/productCommandRequest/productCommandRequest';
  import { columns, searchFormSchema } from './productCommandRequest.data';
  import EditModal from './Edit.vue';

  export default defineComponent({
    // 若需要开启页面缓存，请将此参数跟菜单名保持一致
    name: '产品模型设备下发服务命令属性表维护',
    components: {
      BasicTable,
      PageWrapper,
      TableAction,
    ...
    setup(props) {
      ...
      const [registerTable, { reload, getSelectRowKeys }] = useTable({
        title: t('iot.link.productCommandRequest.productCommandRequest.table.title'),
        api: page,
        columns: columns(),
        formConfig: {
          name: 'ProductCommandRequestSearch',
          labelWidth: 88,

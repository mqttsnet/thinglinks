<template>
  <BasicModal v-bind="$attrs" @register="registerModel" :title="t(`common.title.${type}`)" :maskClosable="false"
    @ok="handleSubmit" :keyboard="true" :bodyStyle="{ height: '300px' }">
...
import { Api, detail, save, update } from '../../../../../api/iot/rule/alarm/channel';
  ...
  setup(_, { emit }) {
    ...
    const [registerModel, { setModalProps: setProps, closeModal: close }] = useModalInner(
      async (data) => {
        ...
        if (unref(type) !== ActionEnum.ADD) {
          ...
          record.token = JSON.parse?.(record?.channelConfig || '{}')?.token;
          record.secret = JSON.parse?.(record?.channelConfig || '{}')?.secret;
          record.appId = JSON.parse?.(record?.channelConfig || '{}')?.appId;
          record.appSecret = JSON.parse?.(record?.channelConfig || '{}')?.appSecret;
    ...
    async function handleSubmit() {
      try {
        ...
        let obj = { ...params };
        console.log('obj:', obj);
        delete obj.channelName;
        delete obj.channelType;
        delete obj.status;
        delete obj.remark;
        params.channelConfig = JSON.stringify(obj);
        if (params.channelType == 1) {

...
const PRODUCT_LOCK_REF = 'refs/thinglinks/product-config-lock/thinglinks-web';
...
# 组件编码
THINGLINKS_COMPONENT_CODE=thinglinks-web
# 组件名称
THINGLINKS_COMPONENT_NAME="ThingLinks Web"
# 组件版本
THINGLINKS_COMPONENT_VERSION=1.0.8
# 包名称
THINGLINKS_NPM_PACKAGE_NAME=thinglinks-web
# 发行编码
...
function createFixture(t, manifest = MANIFEST) {
  ...
  execFileSync(
    'git',
    ['add', '.thinglinks-product.env', 'package.json', 'LICENSE', 'LICENSE-COMMERCIAL'],
    {
      cwd: root,
    },
  );
...
test('非发行身份字段拒绝独立英文版本词和中文版本标识', (t) => {
  const invalidValues = [
    ...
    ['THINGLINKS_COMPONENT_CODE=thinglinks-web', 'THINGLINKS_COMPONENT_CODE=thinglinks-commercial'],
    [
      'THINGLINKS_COMPONENT_NAME="ThingLinks Web"',
      'THINGLINKS_COMPONENT_NAME="ThingLinks Enterprise"',
    ],
    ['THINGLINKS_COMPONENT_NAME="ThingLinks Web"', 'THINGLINKS_COMPONENT_NAME="ThingLinksWebPro"'],
    [
      'THINGLINKS_NPM_PACKAGE_NAME=thinglinks-web',

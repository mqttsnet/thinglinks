...
public class XxlJobServiceImpl implements XxlJobService {
    ...
    private final ExecutorService executorService = new ThreadPoolExecutor(
            ...
            new ThreadFactoryBuilder()
                    .setNamePrefix("xxl-job-log-cleaner-%d")
                    .setDaemon(true)
                    .build(),
    ...
    public ReturnT<String> remove(int id) {
        ...
        // 异步删除日志数据
        this.deleteJobLogsAsync(id);
        return ReturnT.SUCCESS;
    }

    public CompletableFuture<Void> deleteJobLogsAsync(final int id) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                // 处理异常
                logger.error("deleteJobLogsAsync error", e);
            }
        }, executorService);
    }

    @Override
    public ReturnT<String> save(XxlJobInfoVO xxlJobInfo) {
        ReturnT<XxlJobInfo> jobInfoRes = getJobInfo(xxlJobInfo);
        if (jobInfoRes.getCode() != 200) {
            return new ReturnT<>(jobInfoRes.getCode(), jobInfoRes.getMsg());
        }
        XxlJobInfo jobInfo = jobInfoRes.getContent();
        ...
        jobInfo.setTriggerStatus(xxlJobInfo.getTriggerStatus() != null ? xxlJobInfo.getTriggerStatus() : 1);
     ...
     *
     * @param id 主键
     * @return {@link ReturnT<XxlJobInfo>} 任务详情
     */
    @Override
    ...
    private ReturnT<XxlJobInfo> getJobInfo(XxlJobInfoVO xxlJobInfo) {
        XxlJobInfo jobInfo = new XxlJobInfo();
        BeanUtils.copyProperties(xxlJobInfo, jobInfo);
        if (xxlJobInfo.getScheduleTime() != null) {
            jobInfo.setScheduleConf(DateUtil.getCron(xxlJobInfo.getScheduleTime()));
        }
        if (xxlJobInfo.getJobGroupName() != null && !"".equals(xxlJobInfo.getJobGroupName())) {
            XxlJobGroup jobGroup = xxlJobGroupDao.getByAppName(xxlJobInfo.getJobGroupName());
            if (jobGroup == null) {
                return new ReturnT<XxlJobInfo>(ReturnT.FAIL_CODE,

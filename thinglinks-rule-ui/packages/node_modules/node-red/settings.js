...
module.exports = {
    ...
    //  此处的token 是有 nodered Client 的 url上的 access_token="token" 获取的
    adminAuth: {
        tokens: function(token) {
            // 提取出域名部分，以便于维护和修改
            const baseUrl = 'http://127.0.0.1:19100';
            const verifyTokenEndpoint = '/system/user/getInfo';
            const fullUrl = `${baseUrl}${verifyTokenEndpoint}`;

            // 自动根据baseUrl选择使用http还是https网络协议
            const networkProtocol = baseUrl.startsWith('https') ? require('https') : require('http');
            ...
            return new Promise(function(resolve) {
                ...
                networkProtocol.get(fullUrl, options, (response) => {
                    let responseData = '';
                    ...
                    response.on('end', () => {
                        try {
                            ...
                            console.log("响应数据: ", parsedResponse);
                            // 响应编码:200-请求处理成功
                            if (parsedResponse.code === 200) {
                                console.log('Token验证成功');
                                const user = {username: 'admin', permissions: '*'};
                                resolve(user);
                            } else {
                    ...
                    console.error('请求验证token接口出错: ', error.message);
                    resolve(null);
                });
            });
        }

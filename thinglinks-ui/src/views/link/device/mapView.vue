    ...
    methods: {
            ...
            for (const tip of this.tips) {
                if (value === tip.id) {
                    ...
                    this.projectMapMarker = new window.AMap.Marker({
                        position: new window.AMap.LngLat(tip.location.lng, tip.location.lat),
                    });
                    this.lonLat = tip.location.lng + "," + tip.location.lat;
                    this.map.add(this.projectMapMarker); // 添加点标记在地图上
                    this.map.setZoom(13);
                    this.map.setCenter(tip.location);
                    this.$emit("locationChange", [tip.location.lng, tip.location.lat]);
                    this.regeoCode();
                    break;
                }
            }
        ...
        mapClick(e) {
            ...
            if (this.map !== undefined) {
                ...
                this.projectMapMarker = new window.AMap.Marker({
                    // 定义点标记对象
                    position: new window.AMap.LngLat(e.lnglat.lng, e.lnglat.lat),
                });
        ...
        lonLatLocation() {
            ...
            this.getAddress(this.lonLat.split(','))
            this.projectMapMarker = new window.AMap.Marker({
                // 定义点标记对象
                position: new window.AMap.LngLat(this.lonLat.split(',')[0], this.lonLat.split(',')[1]),
            });
            this.map.add(this.projectMapMarker); // 添加点标记在地图上
        },
        // 获取地址

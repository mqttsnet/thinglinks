      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="省市区" label-width="85px">
            <el-select v-model="provinceValue" placeholder="省份" @change="provinceE">
              <el-option v-for="item in province" :key="item.value" :label="item.name" :value="item.name">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span='12'>
          <el-form-item label="地级市" label-width="80px">
            <el-select v-model="cityValue" placeholder="市级" @change="cityE">
              <el-option v-for="item in city" :key="item.value" :label="item.name" :value="item.name">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span='12'>
          <el-form-item label="区县" label-width="80px">
            <el-select v-model="areaValue" placeholder="区县" @change="areaE">
              <el-option v-for="item in area" :key="item.value" :label="item.name" :value="item.name">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <el-col :span="21">
          <el-form-item label="详细地址" label-width="75px">
            <el-select style="width: 97%;margin: auto;" v-model="address" filterable remote reserve-keyword clearable
...
export default {
  ...
  methods: {
    ...
    setProvince(value) {
      ...
      _this.districtSearch.search(adcode, function (status, result) {
        ...
        if (result.districtList[0] !== undefined) {
          _this.map.setCenter([result.districtList[0].center.lng, result.districtList[0].center.lat])
        }
      });
    },
    //远程搜所
    remoteMethod(query) {
      if (query !== "") {
        let queryValue = this.provinceValue + this.cityValue + this.areaValue + query;
        this.loading = true;
        const _this = this;

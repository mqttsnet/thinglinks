...
import com.mqttsnet.thinglinks.generator.config.EntityConfig;
...
import com.mqttsnet.thinglinks.generator.enumeration.ComponentEnum;
import com.mqttsnet.thinglinks.generator.enumeration.SoyComponentEnum;
import com.mqttsnet.thinglinks.generator.enumeration.SqlConditionEnum;
import com.mqttsnet.thinglinks.generator.enumeration.VxeComponentEnum;
import com.mqttsnet.thinglinks.generator.rules.ColumnType;
import com.mqttsnet.thinglinks.generator.rules.DbColumnType;
import com.mqttsnet.thinglinks.generator.rules.NamingStrategy;
import com.mqttsnet.thinglinks.generator.rules.echo.EchoType;
import com.mqttsnet.thinglinks.generator.rules.enumeration.EnumType;
import com.mqttsnet.thinglinks.generator.type.GenConstants;
import com.mqttsnet.thinglinks.generator.utils.inner.CommentUtils;
import lombok.extern.slf4j.Slf4j;

import java.util.List;
import java.util.Map;

import static com.mqttsnet.thinglinks.generator.utils.inner.PackageUtils.getName;
...
public class GenUtils {
    ...
    public static DefGenTableColumn initColumnField(GeneratorConfig generatorConfig, DbType dbType, DefGenTable genTable, Column column) {
        ...
        // 编辑字段
        if (!GenConstants.NOT_EDIT.contains(name) && !column.isPk() && !tableColumn.getIsLogicDeleteField()) {
            tableColumn.setIsEdit(true);
        }
        // 列表字段
        if (!GenConstants.NOT_LIST.contains(name) && !column.isPk() && !tableColumn.getIsLogicDeleteField()) {
            tableColumn.setIsList(true);
        }
        // 查询字段
        if (!GenConstants.NOT_QUERY.contains(name) && !column.isPk() && !tableColumn.getIsLogicDeleteField()) {

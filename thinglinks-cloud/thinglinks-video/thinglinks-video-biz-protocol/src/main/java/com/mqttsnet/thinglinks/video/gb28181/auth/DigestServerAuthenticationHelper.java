 ...
 *
 */
package com.mqttsnet.thinglinks.video.gb28181.auth;
...
public class DigestServerAuthenticationHelper {
    ...
    private String generateNonce() {
        ...
        byte mdbytes[] = messageDigest.digest(nonceString.getBytes(StandardCharsets.UTF_8));
        return HexUtil.encodeHexStr(mdbytes);
    }
    ...
    public boolean doAuthenticateHashedPassword(Request request, String hashedPassword) {
        ...
        if (StrUtil.isBlank(username) || StrUtil.isBlank(realm)) {
            return false;
        }
        ...
        byte[] mdbytes = messageDigest.digest(A2.getBytes(StandardCharsets.UTF_8));
        String HA2 = HexUtil.encodeHexStr(mdbytes);
        ...
        KD += ":" + HA2;
        mdbytes = messageDigest.digest(KD.getBytes(StandardCharsets.UTF_8));
        String mdString = HexUtil.encodeHexStr(mdbytes);
        String response = authHeader.getResponse();


        return mdString.equalsIgnoreCase(response);
    }
    ...
    public boolean doAuthenticatePlainTextPassword(Request request, String pass) {
        ...
        byte mdbytes[] = messageDigest.digest(A1.getBytes(StandardCharsets.UTF_8));
        String HA1 = HexUtil.encodeHexStr(mdbytes);
        log.debug("A1: " + A1);
        log.debug("A2: " + A2);
        mdbytes = messageDigest.digest(A2.getBytes(StandardCharsets.UTF_8));
        String HA2 = HexUtil.encodeHexStr(mdbytes);
        log.debug("HA1: " + HA1);
        log.debug("HA2: " + HA2);
        // String cnonce = authHeader.getCNonce();
        ...
        if (StrUtil.isNotBlank(qop) && "auth".equalsIgnoreCase(qop)) {
            if (nc != -1) {
                KD += ":" + ncStr;
            }
            if (cnonce != null) {

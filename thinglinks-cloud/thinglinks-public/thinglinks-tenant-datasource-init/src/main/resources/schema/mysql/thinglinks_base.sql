...
CREATE TABLE `video_device_group_relation` (
  ...
  `updated_by` bigint DEFAULT NULL COMMENT '更新人',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_group_device_channel_key` (`group_id`,`device_identification`,`channel_identification_key`),
...
CREATE TABLE `video_gateway_mapping` (
  ...
  `updated_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_src_device_channel_key` (`src_protocol`,`src_device_identification`,`src_channel_identification_key`),
